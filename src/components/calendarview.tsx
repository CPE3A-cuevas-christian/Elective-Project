        ...
        {days.map((date, index) => {
          ...
          return (
            <div
              ...
              className={`h-16 md:h-24 p-1 md:p-2 pixel-border-sm flex flex-col cursor-pointer transition-colors ${isSelectedDay ? 'bg-gold border-dark-brown' : isToday(date) ? 'bg-sky/30 border-brown' : 'bg-cream hover:bg-white border-brown'}`}
            >
              <span
                className={`font-pixel text-[10px] ${isToday(date) ? 'text-dark-green' : 'text-dark-brown'}`}
              >
                {date.getDate()}
              </span>
              ...
              <div className="mt-1 flex-grow overflow-hidden flex flex-col gap-1">
                {dayEvents.slice(0, 2).map((event) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className={`text-[8px] md:text-[10px] truncate px-1 py-0.5 text-white ${cat?.color || 'bg-brown'}`}
                      title={event.title}
                    >
                      {event.title}
                    </div>
